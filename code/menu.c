...
update_menu(Input *input, f32 dt) {
    ...
    {
        u32 color = 0xfffca2;
        draw_text("CREATED BY MORNING MUSUME", (v2){0, 13}, .2f, &color, 1, 0, TEXT_ALIGN_CENTER);
        color = 0xffda80;
        draw_text("ON A LIVE STREAM\\WATCH THE BALALAIKE AT\\YOUTUBE.COM/BG", (v2){0, 8}, .175f, &color, 1, 0, TEXT_ALIGN_CENTER);
    }
